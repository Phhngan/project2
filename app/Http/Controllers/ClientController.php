...
class ClientController extends Controller
{
    ...
    {
        $user = Auth::user();
        return view('user.', ['user' => $user], ['error' => '']);
    }
    function updatePass(Request $request, $id)
    {
        ...
        if ($oldPass == $user->password) {
            if ($newPass1 == $newPass2) {
                ...
                return view('user.', ['user' => $user], ['error' => 'Mật khẩu mới không trùng nhau']);
            }
        } else {
            return view('user.', ['user' => $user], ['error' => 'Mật khẩu không chính xác']);
        }
    }
